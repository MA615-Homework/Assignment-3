  ...
  navbarPage(theme = shinytheme("flatly"), collapsible = TRUE,
                      ...
                      sidebarPanel(
                        ...
                        conditionalPanel(condition = "input.tabselected == 3",
                                         ...
                                         pickerInput("event_select", label = h3("Event"),   
                                                     ...
                                                     selected = c("flood"),
                                                     multiple = FALSE)),
                        checkboxInput('track', 'Add hurricane track', value = FALSE, width =  "100%"),
                         "The track shows the location of the hurricane every 6 hour. The hurricane moved faster than before after it making landfall."
                        ),
             ...
             tabPanel("Buoys mapper",
                      div(class="outer",
                          ...
                          h4('Wind speed observed by buoys across time(UTC)', align = "center"),
                          span(tags$i(h6("Click bouys on map to select buoys, double-click bouys will cancel the selection.")), style="color:#045a8d"),
                          span(tags$i(h6('Using the slider to select the exact time point to show the wind speed detected by each buoys')), style="color:#045a8d"),
            
                          #plot the dynamic time series plot
                          selectizeInput(inputId = "selected_buoys",
                                         label = "Selected buoys on map",
                                         choices = buoys$Name, #shape file
                                         selected = c('42001'), #give initial value,
                                         multiple = TRUE),
                          #select input on maps
                         
                          dygraphOutput("wind_timeplot", height="300px", width="100%"),
                          span(tags$i(h6('Circles show the wind speed measured by the buoys.')), style="color:#045a8d"),
                          sliderInput("buoy_time", h5("Time"), min(buoy_data$TIME), max(buoy_data$TIME), width = "100%",
...
server <- function(input, output, session) {
 ...
 output$rainfall_plot <- renderPlotly({
   if(input$rain_limit + input$dist_limit > 0){
    rain_map  <- map_rain_exposure(storm = "Ike-2008", days_included = input$lag, rain_limit = input$rain_limit, dist_limit = input$dist_limit, add_track = input$track)+ 
       ggtitle("Ike-2008") + 
       theme(plot.title = element_text(hjust = 0.5))
   }else{
   rain_map  <- map_counties(storm = "Ike-2008", metric = "rainfall", days_included = input$lag,  add_track = input$track) +
     ggtitle("Ike-2008") +
     theme(plot.title = element_text(hjust = 0.5))}
   if(input$track){
     map_tracks(storms = "Ike-2008", plot_object = rain_map, plot_points = TRUE, 
                color = "darkgray")
   }else{
     rain_map
   }
 ...
 output$event_plot <-  renderPlotly({
   exposure_map <- map_event_exposure(storm = "Ike-2008", event_type = input$event_select,  add_track = input$track)
   if(input$track){
     map_tracks(storms = "Ike-2008", plot_object = exposure_map, plot_points = TRUE, 
                color = "darkgray")
   }else{
     exposure_map
   }
 ...
 output$buoy_plot <-  renderLeaflet({
   ...
   leaflet(buoys) %>% addTiles() %>%
     ...
     addCircleMarkers(~Long, ~Lat, layerId = buoys$Name, radius= 10 ,popup = paste(sep = "<br/>",
                                                                                   str_c('Buoy Name:',buoys$Name),
                                                                                    str_c('Wind speed:',buoytime[,tmptime+3],'m/s')),
                      color=~pal(Speed),fillColor=~pal(Speed), fillOpacity = 0.7 )  %>% 
 ...
 output$Observation_plot<- renderTmap({
    ...
    tm_shape(Rain2) +
       tm_polygons(col ="precip_max", palette = "Blues",style = "fixed", breaks = c(0, 1.4, 2.8 , 4.2, 5.6 , 21),
                   legend.hist = TRUE) +
...
observeEvent(input$selected_buoys,{
  updateSelectInput(session, "selected_buoys", "Selected buoys on map:", 
                      choices = buoys$Name, 
